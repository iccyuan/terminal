...
// anyTerminal patch (microsoft/terminal#18725, PR#19089 re-land): ask the connected
// terminal for its current cursor position (DSR CPR) and adopt it. Called after a resize
// (PtySignalInputThread::_DoResizeWindow) so ConPTY's cursor agrees with the terminal's
// own reflow instead of GetConsoleCursorInfo returning a stale/wrong row.
void VtIo::RequestCursorPositionFromTerminal()
{
    ...
    if (_lookingForCursorPosition)
    {
        // By delaying sending another DSR CPR until we received a response to the previous
        // one, we debounce our requests to the terminal. We don't want to flood it.
